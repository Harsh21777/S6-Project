...
def get_data_nodes(topology, capacity_percent):
    """
    Args:
    - topology (str): Name of the topology.
    - capacity_percent (float): Percentage of node capacity.

    Returns:
    - nodes (DataFrame): Adjusted node information.
    - edges (DataFrame): Information about connections between nodes.
    - s_matrix (np.array): Distance matrix between nodes._
    """
...
def get_data_application(app_name, path):
    ...
    - path (str): Path to the application data.

    Returns:
    - services (DataFrame): Information about the services.
    - choreography (DataFrame): Information about choreographies.
    """
...
def solve_placement(services, nodes, s_matrix, time_limit):
    ...
    Args:
    - services (DataFrame): Information about the services.
    - nodes (DataFrame): Information about the nodes.
    - s_matrix (np.array): Distance matrix between nodes.
    - time_limit (int): Solver time limit (in minutes).

    Returns:
    - allocations (list): Service-to-node allocations.
    - nodes_allocated (list): List of allocated nodes.
    - time_placement (str): Total execution time.
